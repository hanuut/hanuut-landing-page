...
export default function PrePreparedDesignsTab({ onSelectArtwork }) {
  ...
  useEffect(() => {
    ...
    // 🔴 Fetch ALL catalog designs (removed the collection filter)
    axios
  ...
  if (designs.length === 0) {
    return (
      ...
      >
        {t("pod_studio_preprepared_empty", "No designs found.")}
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span>
          {isRtl
            ? "اختر تصميماً آخر من التشكيلة"
            : "Select Alternative Collection Design"}
        </span>
      </div>
      ...
      >
        {designs.map((art) => {
          ...
          const cleanName = (art.originalname || "")
            .split(".")[0]
            .replace(/[_-]/g, " ");
