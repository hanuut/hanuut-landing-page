...
const ModalContent = styled(motion.div)`
  ...
  padding: 2.5rem;
  max-height: 90vh;
  overflow-y: auto; /* FIX: Enable Scroll */
  color: #FFFFFF;
  position: relative;
...
const QuantityButton = styled.button`
  ...
  background-color: transparent; color: white; font-size: 1rem; cursor: pointer;
  display: flex; align-items: center; justify-content: center; transition: all 0.2s;
  &:hover { border-color: #fff; background-color: rgba(255,255,255,0.1); }
`;

const ToggleListButton = styled.button`
  background: transparent; border: none; color: #a1a1aa; font-size: 0.9rem;
  display: flex; align-items: center; justify-content: center; gap: 0.5rem;
  cursor: pointer; width: 100%; padding: 0.5rem; margin-bottom: 1.5rem; border-radius: 8px;
  &:hover { background: rgba(255,255,255,0.05); color: white; }
`;

const TotalContainer = styled.div`
  display: flex; justify-content: space-between; align-items: center;
  margin-top: 1rem; padding-top: 1rem; border-top: 1px solid rgba(255, 255, 255, 0.1); margin-bottom: 1.5rem;
`;
...
const Input = styled.input`
  ...
  border: 1px solid rgba(255, 255, 255, 0.1); background-color: rgba(0, 0, 0, 0.3);
  color: white; border-radius: 12px; box-sizing: border-box; transition: all 0.2s;
  &:focus { outline: none; border-color: ${(props) => props.theme.primaryColor}; background-color: rgba(0, 0, 0, 0.5); }
`;
const TextArea = styled.textarea`
  width: 100%; padding: 0.9rem 1rem; font-size: 0.95rem;
  border: 1px solid rgba(255, 255, 255, 0.1); background-color: rgba(0, 0, 0, 0.3);
  color: white; border-radius: 12px; box-sizing: border-box; resize: vertical; min-height: 80px;
  &:focus { outline: none; border-color: ${(props) => props.theme.primaryColor}; }
`;

const SubmitButton = styled.button`
  width: 100%; padding: 1.1rem; font-size: 1.1rem; font-weight: 700; color: #111; 
  background-color: ${(props) => props.theme.primaryColor}; border: none; border-radius: 16px;
  cursor: pointer; transition: all 0.3s ease; box-shadow: 0 4px 20px ${(props) => `${props.theme.primaryColor}40`};
  &:hover { transform: translateY(-2px); filter: brightness(1.1); }
  &:disabled { opacity: 0.6; cursor: not-allowed; transform: none; box-shadow: none; background-color: #333; color: #777; }
`;

const LocationButton = styled.button`
  display: flex; align-items: center; justify-content: center; gap: 0.5rem;
  width: 100%; padding: 0.9rem; background-color: rgba(255, 255, 255, 0.05);
  border: 1px dashed rgba(255, 255, 255, 0.2); border-radius: 12px; color: #a1a1aa;
  cursor: pointer; font-size: 0.9rem; transition: all 0.2s ease;
  &:hover { background-color: rgba(255, 255, 255, 0.1); color: white; border-color: rgba(255, 255, 255, 0.4); }
  ${props => props.$status === 'success' && css` background-color: rgba(57, 161, 112, 0.15); border-color: #39A170; color: #39A170; `}
  ${props => props.$status === 'loading' && css` opacity: 0.7; cursor: wait; `}
`;
            ...
            <ItemsColumn>
                ...
                {items.length > 3 && (
                    <ToggleListButton type="button" onClick={() => setIsListExpanded(!isListExpanded)}>
                        {isListExpanded ? <><FaChevronUp /> {t('show_less', 'Show Less')}</> : <><FaChevronDown /> {t('show_more_items', `+ ${hiddenCount} more items`)}</>}
                    </ToggleListButton>
                )}
                <FormGroup style={{ marginTop: '1rem' }}>
                    <InputLabel htmlFor="note">{t("form_preparation_note")} ({t("optional")})</InputLabel>
                    <TextArea id="note" value={note} onChange={(e) => setNote(e.target.value)} placeholder={t("note_placeholder", "Any special instructions?")}/>
                </FormGroup>
                <TotalContainer>
    ...
    <>
      <AnimatePresence>
        {items.length > 0 && !isOpen && (
          <FloatingButton onClick={onOpen} initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} $isPremium={isPremium} $accentColor={brandColors?.accent}>
